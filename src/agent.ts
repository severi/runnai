...
- **Check the two-layer shape.** A run analysis opens with a 3-5 line summary — no headers inside it — covering what the session was with its defining numbers, plan vs actual in both type and size, the single most important thing (or "nothing to flag"), and any consequence for what comes next. Detail sections follow under \`## \` headers. Flag at confidence 85 if: the summary is missing or buried under a header; it runs well past 5 lines; or a finding appears in the detail that the summary never mentions — the athlete reads the summary and stops, so a finding only visible below the fold is a finding they will miss.
