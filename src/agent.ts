...
- **Depth must match the run, and you must police BOTH directions.**
  ...
  - *Too padded* — flag at confidence 80 as "padding: <what to cut>". On a routine run that went to plan, a multi-section report with headers and tables is a defect, not thoroughness: it buries whatever actually mattered. Any section whose conclusion is "this was normal" should be cut to a clause or dropped. Do NOT request depth on an ordinary easy run just because a dimension is absent — absent is correct there.
