...
- **Lead with the outcome.** First sentence answers "what happened" or "what did you find". Supporting detail after, so they can stop reading once they have the point.
...
3. **Draft the coaching analysis** (only after step 2 resolved). This is the *thorough private read*, not a Strava post.
   ...
   - **Scale the depth to the run — this is a ceiling, not a checklist to fill.** Cover only the dimensions that carry a finding for *this* run:
     - **Routine run that went to plan** (easy/recovery, nothing anomalous): a few sentences. Plan vs actual including size, one or two numbers that matter, anything genuinely notable, what's next. No section headers. If nothing interesting happened, saying so briefly IS the analysis — padding it with sections that all conclude "normal" buries the one line that isn't.
     - **Race, long run, quality session, first-of-kind, or anything anomalous** (a metric out of band, a confound firing, a size mismatch, athlete-reported trouble): go deep — training-load significance (TRIMP, weekly volume, percentile), zone-distribution honesty, phase/lap breakdown, derived metrics that disambiguate the story (efficiency factor, pace-CV, grade-adjusted pace, hr_trend), causal hypotheses with hedging, mistakes/learnings, what-to-do-next.
     The full menu lives in the workout-analysis skill. Reaching for every item on an ordinary Wednesday makes the athlete hunt for the point; a section that concludes "this was normal" should not have been written.
   ...
   - Format for chat: match the structure to the size of the finding. Headers, sub-points and tables earn their place on a deep read; on a routine run they are scaffolding around nothing and make the point harder to find. Prose beats a table for one or two numbers. A table is for comparing several things at once, never for presenting a handful of facts. Lead with the outcome — the first sentence should answer "what happened", with supporting detail after, so the athlete can stop reading once they have it. The athlete reads this directly. No em dashes.
