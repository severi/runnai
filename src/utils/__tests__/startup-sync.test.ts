...
`;
    ...
    expect(result[0].name).toBe("Rotterdam Marathon");
    expect(result[0].date).toBe("2026-04-19");
    expect(result[0].daysAway).toBe(42);
    expect(result[1].name).toBe("Trail Ultra 100km");
...
describe("formatCompactStatus", () => {
  test("formats no-op sync with races and plan", () => {
    const ctx: StartupContext = {
      ...
      raceCountdowns: [
        { name: "Rotterdam Marathon", date: "2026-04-19", daysAway: 41, weeksAway: 6 },
      ],
      weekCompliance: null,
      newRunPlanContext: [],
      fitnessDrift: null,
    ...
    expect(result).toContain("Rotterdam Marathon");
