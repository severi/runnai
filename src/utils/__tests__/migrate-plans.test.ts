...
describe("migrate-plans", () => {
  test("converts rotterdam-marathon-2026.md to dir layout", async () => {
    const oldPath = path.join(getPlansRoot(), "rotterdam-marathon-2026.md");
    await fs.mkdir(getPlansRoot(), { recursive: true });
    await fs.writeFile(oldPath, "# Rotterdam Marathon → Trail Ultra 100km\n\nbody...");

    const result = await migratePlans();
    expect(result.migrated).toEqual(["rotterdam-marathon-2026"]);

    const slug = "rotterdam-marathon-2026";
    const planFile = await fs.readFile(getPlanFile(slug), "utf-8");
    expect(planFile).toContain("---\ntitle: ");
    expect(planFile).toContain("slug: rotterdam-marathon-2026");
    expect(planFile).toContain("# Rotterdam Marathon");

    expect(await fs.readFile(getVersionPlanFile(slug, 1), "utf-8")).toContain("# Rotterdam Marathon");
