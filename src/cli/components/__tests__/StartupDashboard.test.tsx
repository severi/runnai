...
describe("StartupDashboard", () => {
  ...
  test("renders race countdowns", () => {
    const ctx: StartupContext = {
      ...baseCtx,
      raceCountdowns: [
        { name: "Rotterdam Marathon", date: "2026-04-19", daysAway: 42, weeksAway: 6 },
        { name: "Trail Ultra", date: "2026-07-11", daysAway: 125, weeksAway: 18 },
      ],
    };
    const { lastFrame } = render(<StartupDashboard ctx={ctx} greeting={null} />);
    const output = lastFrame()!;
    expect(output).toContain("Rotterdam Marathon");
    expect(output).toContain("42 days");
    expect(output).toContain("6 weeks");
    expect(output).toContain("Trail Ultra");
  });
  ...
  test("renders plan excerpt current week", () => {
    ...
    const { lastFrame } = render(<StartupDashboard ctx={ctx} greeting={null} />);
    const output = lastFrame()!;
    expect(output).toContain("rotterdam-marathon-2026");
